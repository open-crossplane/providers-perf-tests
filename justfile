...
run_tests: (_run_test azure_provider_pod)

_run_test pod:
  @cd {{uptest}} && go run {{uptest}}/cmd/perf/main.go \
         --mrs {{yaml}}/azure-registry.yaml=3 \
         --provider-pod {{pod}} \
         --provider-namespace upbound-system \
         --node 192.168.13.226:9100 \
         --step-duration 1s

# create arbitrary number of registries
create_registries iter='2':
  #!/usr/bin/env bash
  for ((i = 0; i < {{iter}}; i++)); do
    random_suffix=`echo $RANDOM`
    envsubst < {{yaml}}/azure-registry.yaml | kubectl apply -f -
  done
